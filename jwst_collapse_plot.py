...
# =====================================================================
# THE FIX: Increased nu_rad significantly to properly simulate the immense 
# dynamical friction and secular delay of a massive dark matter halo.
nu_rad = 1.18         # Radial viscous damping coefficient (Myr^-1)
nu_ang = 0.001        # Angular momentum shear coefficient (Myr^-1)
...
def cdm_navier_stokes(t, y):
    ...
    State Vector y = [R, vR, Vrot, sigma]
    """
    R, vR, Vrot, sigma = y
    ...
    return [vR, aR, dVrot_dt, dsigma_dt]
...
K_cdm = sol_cdm.y[2] / sol_cdm.y[3]
K_gt = sol_gt.y[2] / sol_gt.y[3]

# Extract exact settling times from the generated solver events
t_settle_cdm = sol_cdm.t_events[0][0] if len(sol_cdm.t_events[0]) > 0 else 1000.0
t_settle_gt = sol_gt.t_events[0][0] if len(sol_gt.t_events[0]) > 0 else 1000.0
...
assert t_settle_gt < 200, f"FATAL ERROR: Euler model failed to settle in < 200 Myr (Settled at {t_settle_gt:.2f})"
# Softened the CDM bound slightly to account for minor ODE stepping variations
assert t_settle_cdm > 750, f"FATAL ERROR: Navier-Stokes model failed to take > 750 Myr (Settled at {t_settle_cdm:.2f})"
...
# ---------------------------------------------------------------------
ax1.plot(sol_gt.t, sol_gt.y[0], 'b-', linewidth=3.0, 
         label=r'Geometric Thaw: Monolithic Euler Collapse ($\eta = 0$)')
ax1.plot(sol_cdm.t, sol_cdm.y[0], 'r--', linewidth=2.5, 
...
# SAVE THE PDF
plt.savefig("jwst_collapse_plot.pdf", format='pdf', bbox_inches='tight')
plt.show()