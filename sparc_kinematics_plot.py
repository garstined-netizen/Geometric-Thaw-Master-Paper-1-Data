...
print(f"Y_disk = {popt_nfw[0]:.3f}, V200 = {popt_nfw[1]:.2f}, c = {popt_nfw[2]:.2f}")
print(f"Reduced Chi-Square (X^2_v): {chi2_nu_nfw:.3f}\n")

print(f"Oldroyd-B Viscoelastic Metric Model:")
print(f"Y_disk = {popt_ob[0]:.3f}, eta = {popt_ob[1]:.3f}, "
      f"lambda_1 = {popt_ob[2]:.2f}, lambda_2 = {popt_ob[3]:.2f}")
...
Vdisk_plot = np.sqrt(popt_ob[0]) * sparc_data['Vdisk'] 
...
Vbar_only = np.sqrt(np.clip(calc_Vbar_sq(sparc_data['Vgas'], sparc_data['Vdisk'], 
                            sparc_data['Vbulge'], popt_ob[0]), 0, np.inf))
...
# Save the publication-ready PDF 
plt.savefig("sparc_kinematics_plot.pdf", format='pdf', bbox_inches='tight')
plt.show()